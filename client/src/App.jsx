...
function App() {
  ...
  useEffect(() => {
    ...
    const onProgress = (step) => {
      if (step.status === 'streaming') {
        // Only update known agent channels to avoid accidentally writing into prompt
        ...
        if (allowed.includes(step.service)) {
          ...
          // ignore unknown streaming targets
    ...
    // listen to analysis-error (server emits this on analysis failure)
    ...
    return () => {
      ...
      // clear any pending filter log timers
  ...
  const activeOutline = useMemo(() => {
    if (!results) return [];
    const content = activeTab === 'optimal'
      ? results.optimalAnswer
      : activeTab === 'report'
        ? results.validationReport
        : '';
    return getOutline(content);
  }, [results, activeTab]);
  ...
  const validationScore = useMemo(() => {
    if (!results?.validationReport) return null;
    const match = results.validationReport.match(/점수\s*[:\-]?\s*([0-9]{1,3})/);
  ...
  const selectHistory = (item) => {
    if (isAnalyzing) {
      // show toast that selection is blocked (managed with cleanup)
  ...
  const handleSaveToNotion = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE.replace(/\/$/, '')}/api/notion/save`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt, summary: results.summary, results: results.results })
      });
      const data = await res.json();
      if (data?.success) alert('노션 저장 완료! ' + (data.url ? `(${data.url})` : ''));
      else alert('노션 저장 실패');
    } catch (err) {
      console.error(err);
...
<html lang="ko">
  ...
  const RALPHIndicator = ({ phase, label, active, completed }) => (
    <div className={`flex flex-col items-center gap-2 transition-all duration-500 ${active ? 'scale-110 opacity-100' : 'opacity-40'}`}>
      ...
      <span className="text-[9px] font-black uppercase tracking-widest text-[#6b635c]">{label}</span>
    </div>
  );

  const StreamingCard = ({ title, icon: Icon, text, color }) => (
    <div className="bg-[#fffdf9] border-2 border-[#e6e0d8] rounded-[2.5rem] p-6 h-full flex flex-col shadow-xl overflow-hidden group hover:border-[#b48a3c]/30 transition-all">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-xs font-black uppercase tracking-widest text-[#6b635c]">{title}</span>
        {text && <div className="ml-auto flex gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /></div>}
      </div>
      <div className="flex-1 overflow-y-auto text-[11px] font-medium leading-relaxed text-[#7a6f66] custom-scrollbar scrollbar-hide whitespace-pre-wrap italic">
        {text || "분석 대기 중..."}
      </div>
    </div>
  );
  ...
  const ExportButton = ({ type, label }) => (
    <button
      ...
      className={`px-6 py-4 rounded-3xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl flex items-center gap-3 transition-all ${
        isExporting[type] ? 'bg-[#b48a3c] text-white' : 'bg-[#f4eee6] text-[#1f2a44] hover:scale-105'
  ...
  return (
    <div className="min-h-screen bg-[#f5f2ed] text-[#1f1f1f] font-sans selection:bg-[#b48a3c]/30">
      <div className="max-w-7xl mx-auto p-6 md:p-12 space-y-12">
        ...
        <header className="flex flex-col xl:flex-row items-center justify-between gap-12">
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-8">
            <div className="relative">
              ...
              <div className="relative bg-gradient-to-br from-[#1f2a44] to-[#2f3b59] p-5 rounded-[2.5rem] shadow-2xl"><Layers className="w-10 h-10 text-white" /></div>
            </div>
            <div>
              ...
              <div className="flex-1 relative group">
                ...
                <div className="relative bg-[#fffdf9] border-2 border-[#e6e0d8] rounded-[2rem] p-3 flex items-center shadow-2xl transition-all focus-within:border-[#b48a3c]/60">
                  <div className="flex items-center gap-3 flex-1">
                    <input type="text" value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="분석할 주제를 입력하세요" className={`flex-1 bg-transparent border-none outline-none px-6 py-3 font-bold text-lg placeholder-[#b0a79f] ${isAnalyzing ? 'opacity-60 cursor-not-allowed' : ''}`} onKeyDown={(e) => e.key === 'Enter' && handleAnalyze()} readOnly={isAnalyzing} aria-readonly={isAnalyzing} />
                    {isAnalyzing && (
                      <div className="flex items-center gap-2 text-[#8a8178] ml-2 pointer-events-none">
              ...
              {agentOptions.map(agent => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-xs font-black border transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 ${
          ...
          <div className="fixed sm:absolute top-2 sm:top-6 right-2 sm:right-6 flex flex-col items-end gap-2 pointer-events-none z-50">
            ...
            {import.meta.env.DEV && (
              <button onClick={() => addFilterLog('Ignored streaming: debug_test', 4000)} className="ml-2 bg-[#b48a3c] text-white px-2 py-1 rounded text-xs font-bold pointer-events-auto">DBG</button>
            )}
          </div>
        </header>
        ...
        {isAnalyzing && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#fffdf9] border-2 border-[#e6e0d8] rounded-[3rem] p-10 shadow-2xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <aside className="lg:col-span-3">
            <div className="bg-[#fffdf9] border-2 border-[#e6e0d8] rounded-[3rem] p-8 h-[800px] flex flex-col shadow-2xl relative overflow-hidden">
                ...
                <AnimatePresence mode="wait">
                  {activeSidebarTab === 'live' ? (
                    <div className="space-y-10">
                      {timeline.map((s, i) => (
                        <div key={i} className="flex gap-6 group">
                          ...
                          <div className="py-1">
                            <span className="text-[10px] font-black text-[#b0a79f] group-last:text-[#1f2a44]">{s.time}</span>
                            <p className={`text-[13px] font-bold mt-1 leading-relaxed ${i === timeline.length - 1 ? 'text-[#1f2a44]' : 'text-[#6b635c]'}`}>{s.message}</p>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {history.map(h => (
                        <div key={h.id} onClick={() => selectHistory(h)} className="p-6 bg-[#f8f4ee] border border-[#e6e0d8] rounded-3xl cursor-pointer hover:bg-[#f1ebe2] transition-all">
                          <p className="text-xs font-black line-clamp-2 mb-3">{h.prompt}</p>
                          <div className="flex justify-between items-center opacity-40"><span className="text-[9px] font-bold">{new Date(h.created_at).toLocaleDateString()}</span><Trash2 className="w-3.5 h-3.5" /></div>
                        </div>
                      ))}
                    </div>
                  )}
          ...
          <main className="lg:col-span-9 space-y-12">
            ...
            {isAnalyzing && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-[380px]">
                {enabledAgents.perplexity && <StreamingCard title="검색 (Perplexity)" icon={Zap} color="sky" text={liveResults.perplexity} />}
                {enabledAgents.chatgpt && <StreamingCard title="추론 (ChatGPT)" icon={Bot} color="emerald" text={liveResults.chatgpt} />}
                {enabledAgents.gemini && <StreamingCard title="창의 (Gemini)" icon={Sparkles} color="indigo" text={liveResults.gemini} />}
                {enabledAgents.claude && <StreamingCard title="검증 (Claude)" icon={Brain} color="amber" text={liveResults.claude} />}
              </div>
            )}
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-8">
                  <div className="flex p-2 bg-[#fffdf9] border-2 border-[#e6e0d8] rounded-[2.5rem] shadow-2xl">
                    {[{ id: 'optimal', icon: BarChart3, label: '종합 인텔리전스' }, { id: 'individual', icon: Layout, label: '에이전트 원본' }, { id: 'report', icon: Brain, label: '논리 검증 보고서' }].map(t => (
                      <button key={t.id} onClick={() => setActiveTab(t.id)} className={`relative px-10 py-5 rounded-[1.8rem] text-[12px] font-black uppercase tracking-widest transition-all z-10 ${activeTab === t.id ? 'text-white' : 'text-[#8a8178]'}`}>
                  ...
                  <div className="flex flex-wrap items-center gap-3 justify-end">
                    ...
                    <button
                      ...
                      className={`px-10 py-5 rounded-3xl font-black text-xs uppercase tracking-[0.2em] shadow-2xl flex items-center gap-4 transition-all ${
                        isNotionSaving ? 'bg-[#b48a3c] text-white' : 'bg-[#1f2a44] text-white hover:scale-105 active:scale-95'
                      }`}
                      disabled={isNotionSaving}
                    >
                      {isNotionSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Share2 className="w-4 h-4" />} Notion \uB0B4\uBCF4\uB0B4\uAE30
                    </button>
                  </div>
                </div>
                    ...
                    <AnimatePresence mode="wait">
                      {activeTab === 'optimal' && (
                        <motion.div key="opt" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                          <div className="mb-8">
                            <h2 className="text-2xl font-black text-[#1f2a44]">종합 인텔리전스 보고서</h2>
                            <div className="text-xs text-[#8a8178] mt-1">생성 시각: {reportTimestamp} · 활성 에이전트: {enabledAgentNames}</div>
                            <div className="h-px bg-[#e6e0d8] mt-4" />
                          </div>
                        ...
                        <motion.div key="ind" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                          {Object.entries(results.results || {}).map(([k, v]) => (
                            <div key={k} className="p-8 bg-slate-50 dark:bg-white/5 rounded-[2.5rem] border border-slate-100 dark:border-white/10">
                              <h3 className="text-[10px] font-black text-[#1f2a44] uppercase tracking-widest mb-4 border-b pb-2">{k} AGENT RESPONSE</h3>
                      ...
                      {activeTab === 'report' && (
                        <motion.div key="rep" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                          <div className="mb-8">
                            <h2 className="text-2xl font-black text-[#1f2a44]">논리 검증 보고서</h2>
                            <div className="text-xs text-[#8a8178] mt-1">생성 시각: {reportTimestamp} · 활성 에이전트: {enabledAgentNames}</div>
                            <div className="h-px bg-[#e6e0d8] mt-4" />
                          </div>
            ...
            {!isAnalyzing && !results && (
              <div className="h-[650px] border-4 border-dashed border-slate-100 dark:border-white/5 rounded-[5rem] flex flex-col items-center justify-center space-y-10 group overflow-hidden relative">
                <div className="absolute inset-0 bg-[#b48a3c]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-1000 blur-[100px]"></div>
                ...
                <div className="text-center space-y-4">
                  <p className="text-[11px] font-black uppercase tracking-[0.6em] text-[#b0a79f]">Multi Agent Analysis 대기</p>
                  <p className="text-xs font-bold text-[#8a8178] italic">고정밀 분석 준비 완료</p>
                </div>
              </div>
            )}
          </main>
      ...
      <style dangerouslySetInnerHTML={{
        ...
        .prose h1, .prose h2, .prose h3 { font-weight: 900; letter-spacing: -0.05em; color: #1f2a44; margin-top: 2.5rem; margin-bottom: 1.5rem; }
        .prose p { line-height: 1.9; font-weight: 500; margin-bottom: 1.8rem; font-size: 1.05rem; text-align: left; }
        .prose strong { color: #b48a3c; font-weight: 900; }
        .prose hr { border: 0; height: 1px; background: linear-gradient(to right, transparent, #b48a3c, transparent); margin: 3rem 0; opacity: 0.3; }
        .prose table { width: 100%; border-collapse: collapse; margin: 1.5rem 0; font-size: 0.95rem; }
        .prose th, .prose td { border: 1px solid #e6e0d8; padding: 0.6rem 0.8rem; vertical-align: top; }
        .prose th { background: #f4eee6; text-align: left; font-weight: 800; color: #1f2a44; }
        .prose td { color: #4b433d; }
      `}} />
    </div>
  );
}
